...
function resolveGeminiProxyBase() {
  return resolveSafeProxyBase({
    envBase: import.meta.env.VITE_GEMINI_PROXY_BASE,
...
function resolveInitialVoiceProvider() {
  if (typeof window === 'undefined') return 'elevenlabs'
  const params = new URLSearchParams(window.location.search)
  const queryVoiceProvider = params.get('voiceProvider')
  if (queryVoiceProvider) return queryVoiceProvider
  return isLocalBrowserHost(window.location.hostname) ? 'elevenlabs' : 'browser-speech'
}

function buildInitialSpeechStatus(voiceProvider) {
  if (voiceProvider === 'browser-speech') {
    return 'Public/demo-safe voice path armed — browser speech is selected by default so GitHub Pages previews do not depend on a private TTS proxy'
  }
  return 'API-only voice path armed — backend TTS provider selected; browser/system speech remains fallback-only and is not used in API mode'
}
...
export default function App() {
  ...
  const [voiceProvider, setVoiceProvider] = useState(() => resolveInitialVoiceProvider())
  const [voiceId, setVoiceId] = useState('browser-default')
  const [speechStatus, setSpeechStatus] = useState(() => buildInitialSpeechStatus(resolveInitialVoiceProvider()))
  ...
  const handleVoiceProviderChange = (nextVoiceProvider) => {
    setVoiceProvider(nextVoiceProvider)
    setSpeechStatus(buildInitialSpeechStatus(nextVoiceProvider))
  }
