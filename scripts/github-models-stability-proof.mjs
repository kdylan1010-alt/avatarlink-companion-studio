...
;(async () => {
  ...
  for (const prompt of prompts) {
    ...
    await page.waitForFunction((count) => {
      const responses = window.__avatarlinkLastResponses || []
      return Array.isArray(responses) ? responses.length >= count : false
    }, beforeCount + 1, { timeout: 1000 }).catch(() => {})
  ...
  const screenshotPath = path.join(artifactsDir, 'github-models-stability-proof.png')
  const outputPath = path.join(artifactsDir, 'github-models-stability-proof.json')
  ...
  const output = {
    ...
    prompts,
    attemptCount: prompts.length,
    successCount: responses.filter((entry) => entry.status === 200).length,
    responses,
    consoleErrors,
    screenshotPath,
  }
